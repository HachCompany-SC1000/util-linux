...
hw_clock_is_utc(const bool utc, const bool local_opt,
	...
	if (debug)
		printf("Assuming hardware clock is kept in %s time.\n",
		       ret ? "UTC" : "local");
	return ret;
}
  ...
  struct stat statbuf;  /* We don't even use the contents of this. */
	...
	if (line3[0]) {
	  fprintf(stderr,
		  ...
		  MYNAME);
	  fprintf(stderr, "(Expected: `UTC' or `LOCAL' or nothing.)\n");
	}
      }
    }
    ...
    if (debug) {
      printf("Last drift adjustment done at %ld seconds after 1969\n",
             (long) adjtime_p->last_adj_time);
      printf("Last calibration done at %ld seconds after 1969\n",
             (long) adjtime_p->last_calib_time);
      printf("Hardware clock is on %s time\n",
	     (adjtime_p->local_utc == LOCAL) ? "local" :
	     (adjtime_p->local_utc == UTC) ? "UTC" : "unknown");
    }
...
-----------------------------------------------------------------------------*/
  ...
  if (mktime_result == -1) {
    ...
    *systime_p = mktime_result;
    if (debug)
      printf("Hw clock time : %4d/%.2d/%.2d %.2d:%.2d:%.2d = "
...
-----------------------------------------------------------------------------*/
  ...
  if (debug)
    printf ("Time read from Hardware Clock: %4d/%.2d/%.2d %02d:%02d:%02d\n",
	    tm.tm_year+1900, tm.tm_mon+1, tm.tm_mday,
            tm.tm_hour, tm.tm_min, tm.tm_sec);
...
----------------------------------------------------------------------------*/
  ...
  if (testing)
    printf("Clock not changed - testing only.\n");
  else {
    if (badyear) {
      /*
...
----------------------------------------------------------------------------*/
	...
	if (strlen(date_opt) > sizeof(date_command) - 50) {
		fprintf(stderr, "--date argument too long\n");
		return 13;
	}
	...
	sprintf(date_command, "date --date=\"%s\" +seconds-into-epoch=%%s",
		date_opt);
	if (debug)
		printf("Issuing date command: %s\n", date_command);

	date_child_fp = popen(date_command, "r");
	if (date_child_fp == NULL) {
		outsyserr("Unable to run 'date' program in /bin/sh shell. "
			    "popen() failed");
		return 10;
	}
	...
	if (debug)
		printf("response from date command = %s\n", date_resp);
	if (strncmp(date_resp, magic, sizeof(magic)-1) != 0) {
		fprintf(stderr, "The date command issued by %s returned "
				  "unexpected results.\n"
				  "The command was:\n  %s\n"
				  "The response was:\n  %s\n",
		...
		if (rc < 1) {
			fprintf(stderr,
				"The date command issued by %s returned "
...
/*------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------*/
	if (!hclock_valid) {
		if (debug)
			printf("Not adjusting drift factor because the "
				 "Hardware Clock previously contained "
				 "garbage.\n");
	} else if (adjtime_p->last_calib_time == 0) {
		if (debug)
			printf("Not adjusting drift factor because last "
				 "calibration time is zero,\n"
				 "so history is bad and calibration startover "
				 "is necessary.\n");
	} else if ((hclocktime - adjtime_p->last_calib_time) < 23 * 60 * 60) {
		if (debug)
			printf("Not adjusting drift factor because it has "
		...
		if (debug)
			printf("Clock drifted %.1f seconds in the past "
...
-----------------------------------------------------------------------------*/
  ...
  if (adjtime.dirty) {
	...
	err = 1;
      ...
      if (err)
	fprintf(stderr, "Drift adjustment parameters not updated.\n");
    }
  }
}
...
determine_clock_access_method(const bool user_requests_ISA) {
  ...
  if (debug) {
	  if (ur)
		  ...
		  printf("No usable clock interface found.\n");
  }
}
   ...
   */
    ...
    if (getepoch) {
      ...
      if (get_epoch_rtc(&epoch, 0))
        fprintf(stderr, "Unable to get the epoch value from the kernel.\n");
      else
      ...
      if (epoch_opt == -1)
        ...
        printf("Not setting the epoch to %d - testing only.\n",
	       epoch_opt);
...
main(int argc, char **argv) {
	...
	if (argc > 0) {
		usage("%s takes no non-option arguments.  "
			"You supplied %d.\n",
		      MYNAME, argc);
	}
	...
	if (utc && local_opt) {
		fprintf(stderr, "%s: The --utc and --localtime options "
	...
	if (adjust && noadjfile) {
		fprintf(stderr, "%s: The --adjust and --noadjfile options "
				  "are mutually exclusive.  You specified "
				  "both.\n", MYNAME);
		exit(EX_USAGE);
	}
	...
	if (noadjfile && !(utc || local_opt)) {
		fprintf(stderr, "%s: With --noadjfile, you must specify "
				  "either --utc or --localtime\n", MYNAME);
		exit(EX_USAGE);
	}
	...
	if (set) {
		...
		/* (time-consuming) */
		if (rc != 0) {
			fprintf(stderr, "No usable set-to time.  "
					  "Cannot set clock.\n");
			exit(EX_USAGE);
		}
	}
	...
	if (getuid() == 0)
		...
		if (set || hctosys || systohc || adjust) {
			...
			fprintf(stderr,
				"Sorry, only the superuser can change the "
