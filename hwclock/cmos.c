       ...
       unsigned long arg)
{
  ...
  fprintf(stderr, ("%s: atomic %s failed for 1000 iterations!"), progname, name);
  exit(1);
}
...
get_permissions_cmos(void) {
  ...
  if (use_dev_port) {
    if ((dev_port_fd = open("/dev/port", O_RDWR)) < 0) {
      int errsv = errno;
      fprintf(stderr, ("Cannot open /dev/port: %s"), strerror(errsv));
      rc = 1;
    } else
      rc = 0;
  } else {
    rc = i386_iopl(3);
    if (rc == -2) {
      fprintf(stderr, ("I failed to get permission because I didn't try.\n"));
    } else if (rc != 0) {
      rc = errno;
      fprintf(stderr, ("%s is unable to get I/O port access:  "
              "the iopl(3) call failed.\n"), progname);
      if(rc == EPERM && geteuid())
        fprintf(stderr, ("Probably you need root privileges.\n"));
    }
  }
  return rc ? 1 : 0;
}
