...
open_rtc_or_exit(void) {
	...
	if (rtc_fd < 0) {
		outsyserr("open() of %s failed", rtc_dev_name);
		exit(EX_OSFILE);
	}
	return rtc_fd;
}
...
do_rtc_read_ioctl(int rtc_fd, struct tm *tm) {
	...
	if (rc == -1) {
		perror(ioctlname);
		fprintf(stderr, "ioctl() to %s to read the time failed.\n",
			rtc_dev_name);
		exit(EX_IOERR);
	}
...
-----------------------------------------------------------------------------*/
  ...
  if (debug)
    printf("Waiting in loop for time from %s to change\n",
	   rtc_dev_name);
  ...
  if (rtc_fd == -1) {
    ...
    if (rc == -1 && (errno == ENOTTY || errno == EINVAL)) {
      /* This rtc device doesn't have interrupt functions.  This is typical
         ...
         */
      if (debug)
	      printf(("%s does not have interrupt functions. "),
		     rtc_dev_name);
      ...
      ret = 1;
      if (rc == -1)
        outsyserr(("read() to %s to wait for clock tick failed"),
		  rtc_dev_name);
      else
        ret = 0;
#else
	 ...
	 interrupt comes and a bootscript with a hwclock call hangs */
      ...
      ret = 1;
      if (rc == -1)
        outsyserr(("select() to %s to wait for clock tick failed"),
		  rtc_dev_name);
      else if (rc == 0)
	fprintf(stderr, ("select() to %s to wait for clock tick timed out\n"),
			  rtc_dev_name);
      else
        ret = 0;
#endif
      ...
      if (rc == -1)
        outsyserr(("ioctl() to %s to turn off update interrupts failed"),
		  rtc_dev_name);
    } else {
      outsyserr(("ioctl() to %s to turn on update interrupts "
...
/*-------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
	...
	if (rc == -1) {
		perror(ioctlname);
		fprintf(stderr, ("ioctl() to %s to set the time failed.\n"),
			rtc_dev_name);
		exit(EX_IOERR);
	}

	if (debug)
		printf(("ioctl(%s) was successful.\n"), ioctlname);
...
----------------------------------------------------------------------------*/
  ...
  if (rtc_fd < 0) {
    if (!silent) {
      if (errno == ENOENT)
        ...
        outsyserr(("Unable to open %s"), rtc_dev_name);
    }
    return 1;
  }
  ...
  if (ioctl(rtc_fd, RTC_EPOCH_READ, epoch_p) == -1) {
    if (!silent)
      outsyserr(("ioctl(RTC_EPOCH_READ) to %s failed"), rtc_dev_name);
    close(rtc_fd);
    return 1;
  }

  if (debug)
	  printf(("we have read epoch %ld from %s "
...
----------------------------------------------------------------------------*/
  ...
  if (debug)
    printf(("setting epoch to %ld "
	   "with RTC_EPOCH_SET ioctl to %s.\n"), epoch, rtc_dev_name);

  if (ioctl(rtc_fd, RTC_EPOCH_SET, epoch) == -1) {
    if (errno == EINVAL)
      fprintf(stderr, ("The kernel device driver for %s "
	      "does not have the RTC_EPOCH_SET ioctl.\n"), rtc_dev_name);
    else
      outsyserr(("ioctl(RTC_EPOCH_SET) to %s failed"), rtc_dev_name);
    close(rtc_fd);
    return 1;
  }
